...
func testAccPreCheck(t *testing.T) {
	if v := os.Getenv("NETAPP_EMAIL"); v == "" {
		t.Fatal("NETAPP_EMAIL must be set for acceptance tests")
	}

	if v := os.Getenv("NETAPP_PASSWORD"); v == "" {
		t.Fatal("NETAPP_PASSWORD must be set for acceptance tests")
	}
